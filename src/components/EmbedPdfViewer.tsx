...
export default function EmbedPdfViewer({ pdfBlobUrl, isLoading = false }: ViewerProps) {
  return (
    <div className="relative w-full h-full island-shell rounded-2xl overflow-hidden">
      {/* Loading overlay */}
      {isLoading && (
      ...
      {!pdfBlobUrl && !isLoading && (
        <div className="flex h-full min-h-[400px] flex-col items-center justify-center gap-4 text-center px-8">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--sea-ink-soft)] mt-1">
              Upload a PDF from the file queue on the left — the live preview will appear here
              automatically.
            </p>
          </div>
        </div>
